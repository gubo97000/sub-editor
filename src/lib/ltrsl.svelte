<script lang="ts">
	...
	import { llmQueryManager } from './stores/llmQueries.svelte';
	...
	// const clickHandler = async (customOptions: CustomOptions) => {
	// 	fetchEndNum = 0;
	// 	fetchNum = 0;
	...
	// 			return fetch('/api/translate-stream', { method: 'POST', body: formData }).then((res) => {
	// 				fetchEndNum += 1;
	// 				return res.formData();
	// 			});
	// 		});
	...
	// 			const result = JSON.parse(r.get('data') as string);
	// 			console.log(result);
	...
	const newClickHandler = async () => {
		const toSend = subState.subs[0]?.content.cues?.map((sub) => {
			return sub.text;
		});
		llmQueryManager.startQuery(
			toSend,
			{ model: cLLME.value[cps.lastSelectedModel], commands: cps.commands },
			(llmAnswer) => {
				let response: string[] = JSON.parse(llmAnswer);
				subState.subs[1] = JSON.parse(JSON.stringify(subState.subs[0]));
				subState.subs[1] = {
					...subState.subs[1],
					id: 'fi-FI',
